...
|---|---|---|
| `backend/**` | B | Go service, schema, migrations |
...
`contracts/openapi.yaml` (Lane A owns) · `backend/migrations/**` (Lane B owns) ·
...
| **1 · Foundations** | Gate A | **3** | `wt/v2-backend-foundation`, `wt/v2-mobile-domain`, `wt/v2-port-validation` | B1–B4 · C1–C2 · E1 | `lib/data/schema.dart` (C only); `backend/migrations` (B only) | Gate C |
...
|---|---|---|---|---|---|---|---|
| **B1** | Fiber v3 app boots; config boundary; `/healthz` | `backend/cmd`, `backend/internal/config`, `internal/api` | A1 | `backend/**` | C1, E1 | `go build`, `go test` | Foundation · **done** |
| **B2** | Go domain types + invariant tests | `backend/internal/domain` | A1 | `backend/**` | C1, E1 | `go test ./internal/domain` | Foundation · **done** |
| **B3** | Storage interfaces + in-memory implementation | `backend/internal/storage` | B2 | `backend/**` | C1, E1 | `go test ./internal/storage` | Foundation · **done** |
| **B4** | Fresh Postgres schema as migrations | `backend/migrations` | B2 | `backend/migrations` | C2, E1 | Schema-shape test parses SQL and asserts every domain table and constraint | Foundation · **done** |
| **B5** | Postgres repository over pgx | `backend/internal/storage/postgres` | B4, Gate B | `backend/**` | C3+, F1+ | Integration tests, skipped without `DATABASE_URL` | Functionality |
| **B6** | Revision counter and `GET /changes` feed | `internal/api`, `internal/sync` | B5 | `backend/**` | C, F, E | Cursor tests over interleaved creates, updates, tombstones | Functionality |
| **B7** | `POST /mutations` with the idempotency ledger | same | B5 | `backend/**` | C, F, E | Duplicate mutation ids produce one effect | Functionality |
| **B8** | Identity arbitration: evidence → canonical or `unresolved` | `internal/identity` | B5, A4 | `backend/**` | C, F, E | Convergence tests **including the cases that must not merge** | Functionality |
| **B9** | Ordinal placement arbitration | `internal/identity` | B8 | `backend/**` | C, F, E | Two-device conflicting placement detected centrally | Functionality |
| **B10** | DownloadRequest create, claim, resolve | `internal/api` | B7 | `backend/**` | D, G | Single-winner claim; at most one non-terminal per `(library, entry)` | Functionality |
| **B11** | Dev library namespace middleware behind `SCROLLARY_DEV_MODE` | `internal/api` | B1 | `backend/**` | anything | Server refuses to start without the flag | Functionality |
...
| **B4** — `backend/migrations` | Ordered, append-only. Two agents produce two migration `0002`s |
...
| **Must not modify** | `backend/**`, `lib/**`, `pubspec.yaml` |
