...
## 7. One more question, and only when the files are refused

Everything above is asked *before* a run. There is one question the flow may
ask **during** one, and it exists because some sites hand their pictures to the
browser and to nothing else: cross-origin, no `Access-Control-Allow-Origin`,
and a human-verification interstitial for any separate client. Such a page used
to stop with `StopReason.assetsRefusedBySource` and commit nothing, which is
still what happens by default.

**What is offered.** A copy of the reading as the browser *drew* it — the
pixels already on screen, which is what the device's own screenshot key
takes — rather than the site's own files. It is reached from the refusal points
in `SaveEngine` and nowhere else, so it is never an alternative to downloading;
by the time it is offered the ordinary path has been exhausted, not skipped.

**When it is asked.**

| Condition | What happens |
|---|---|
| No answerer wired (`SaveEngine.renderedConsent` is null) | The capture stops with its named reason. Silence is not consent |
| The page's image content band cannot be established | Never asked. Without the band there is no way to tell the reading from the comments below it, so there is nothing honest to keep |
| This Source already answered | That answer is obeyed — including *no*. A declined Source is not asked again on its next Entry |
| Otherwise | Asked once, for the Source; both answers are recorded |

The answer is **device-local** (`local_settings`, keyed by the Source, or by
host for a page no Source has adopted yet). It sits beside the finished-copy
rule for the same reason: it decides what is kept as bytes on *this* device.

**What it writes.** An `OfflineCopy` like any other, with
`manifest.renderedFromPage` set. The byte-for-byte rule still governs
`imageSequence` packages of originals, and a rendering is never recorded as
one — the reader says which it is holding, and a later re-save can tell that
real files would be an improvement.

**What it costs the next save.** Discovering a refusal costs one request per
panel, so what this device watched an origin do is remembered in
`asset_origins` — one refused reading is a suspicion that changes nothing, two
on separate Locations reduce the next reading's discovery to a single request,
and a verdict goes stale after a fortnight. It is device-local, never synced,
never seeded, and it is not a site list: the only thing it changes is how many
times the app re-asks a question already answered. The full rule, and the
measurements behind each constraint, are DECISIONS.md V2-D74.
