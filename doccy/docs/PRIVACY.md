...
| Saved page bytes (text + images) | `webread/library/…` in app-private storage, excluded from device backup. Where a site refused its own files and the user consented, the package holds a **rendering of the page as the browser drew it** instead, marked `renderedFromPage` in its manifest (V2-D74) | Remove offline files (per entry, per collection, all finished), which keeps the rows; *Delete permanently*, which removes the collection's files and its rows together |
...
| What an asset host was observed to do | `asset_origins` — `scheme://host[:port]` and whether this device saw it refuse the pictures it serves. Empty until a save is refused; a verdict expires after 14 days. No page content, no address, nothing about what was read (V2-D74) | expires on its own; the internal-build full reset |
| What this device's last update check concluded | `collection_check_states` — a Collection id, when its check concluded and whether it failed. Never what it found | *Delete permanently* for the collection; the internal-build full reset |
| Preferences | `settings` — including the "Keep working while I read" preference, each Source's rendered-fallback answer, and each Collection's finished-copy rule | the internal-build full reset |
