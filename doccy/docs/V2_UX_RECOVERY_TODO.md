...
> **All ten are done. This is a completed record, not outstanding work** — keep
> it for the root causes, which are the reason several standing rules exist, and
> do not read the "fix" paragraphs as a plan. Where later work moved one of
> these answers, the item says so in place.
...
| 10 | The routine flow stays short | DONE |
...
**Since superseded on *where* it is kept (V2-D73).** It was written to
`LocalSettingsStore`, keyed by Collection, because the schema was frozen. It is
now `collections.capture_mode` and it **synchronises**, as an opaque token the
service stores and never interprets: someone who said "always images" said it
about the Collection, not about the phone they said it on.
`adoptLegacyCollectionPreferences` moves a value an older build wrote onto the
Collection row. The rule this item is really about — a preference *proposes*
and the page disposes — is unchanged.
