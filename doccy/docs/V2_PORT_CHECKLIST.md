...
- **Intentional internal changes:** none to enumeration. Its host file
  ...
  first change on this list to move a judgement rather than a seam. A second
  followed: `LazyImageRuns` judges an unloaded placeholder by the run it sits
  in rather than by its own box (§17.6, 2026-09-03), which is **traversal's
  rule and never selection's** — the traversal set stays a superset of the
  selected one, and `selectImageCandidates` rejects a still-unloaded
  placeholder on the settled page exactly as before.
...
- **New location:** unchanged.
- **Intentional internal changes:** none to settling itself. Its host file
  `lib/save/save_engine.dart` carries the reviewed result seam (§17.1,
  ...
  own. What a *pending* image is has been widened once, reviewed (§17.6,
  2026-09-03): a placeholder is judged by its run, so settling keeps waiting on
  a column of unloaded panels it previously read as a column of icons.
...
- **New location:** unchanged.
- **Intentional internal changes:** one, reviewed — the durable
  `renderedFromPage` flag (§17.7, 2026-09-03). It is an added optional field,
  not a change to how a version is read: a manifest without it reads exactly as
  before.
...
- **New location:** unchanged.
- **Intentional internal changes:** one, reviewed — the added
  `StopReason.assetsRefusedBySource` (§17.6, 2026-09-03). It is a new named
  outcome in a file whose whole design is named outcomes; no existing reason,
  threshold or ordering moved.
...
- **New location:** unchanged.
- **Intentional internal changes:** one, reviewed — refusal is classified as
  an answer rather than retried (§17.6, 2026-09-03). Unchanged and unchangeable:
  it must never import `capture_policy.dart`, and the image-only MIME
  allow-list is a separate rule that stays.
...
- **Device assumptions:** image bytes verified by magic number, never by
  ...
  CORS-closed has no working path **to its files** and is recorded as such.
  Since 2026-09-03 that is no longer where the save ends — with the user's
  consent the reading is kept as the browser drew it instead (V2-D74,
  `lib/save/rendered_capture.dart`). Nothing in this file participates: it is
  reached from `SaveEngine`'s refusal points, and the fetcher's own boundary is
  unchanged. Stored
...
- **Intentional internal changes:** two, reviewed together — `innerText` is
  trusted only where it is the layout-aware reading, and a page's own menus are
  excluded from its prose measurement (§17.8, 2026-09-03).
...
- **Intentional internal changes:** one, reviewed — the origin and
  registrable-domain helpers the asset-origin memory is keyed by (§17.7,
  2026-09-03). Additive: no existing normalisation rule was touched.
...
edit.* Eight such changes have now been reviewed and made. §17.1 and §17.2 cut
...
**§17.5 onwards do.** They are the entries that change what the ported code
*decides*, and each is recorded at that weight: a rule that was wrong on a real
page, the evidence it was wrong, and what was deliberately left alone. §17.6,
§17.7 and §17.8 were all forced by the same week of real-site work, and each
one names the page that disproved the previous rule.
...
### 17.6 A refusal is an answer, and a placeholder is judged by its run — 2026-09-03

**Files:** `lib/save/asset_fetcher.dart`, `lib/save/stop_conditions.dart`,
`lib/save/image_candidates.dart`, `lib/save/page_stability.dart`.

**Two rules, both wrong on the same real page**, and both about mistaking a
settled answer for a bad moment.

**The fetcher retried refusals.** A host that replies 401, 402, 403, 407, 429
or 451, or serves a *web page* where an image was asked for, has settled the
question. The fetcher treated those as transient and retried, which on a
reading of a hundred and thirty panels is a hundred and thirty repetitions of a
question already answered — the "retry with different headers" rule one step
removed. Refusal is now classified as `AssetFailure.refused`: retries stop, and
the only remaining attempt is through the page's own session, which is the one
context that legitimately holds whatever the user established by browsing
there.

The outcome needed a name, so `stop_conditions.dart` gained
`StopReason.assetsRefusedBySource`. **A reading whose images were refused does
not become a partial entry:** where more assets were refused than were stored,
the capture commits nothing and says one sentence about what the site does.
*Retry failed* on such an entry could only be refused again. The comparison is
against what was actually *stored*, so a refusal that did not prevent the entry
from being saved is still an ordinary broken asset and still yields a
`partial` — one dead panel among a hundred good ones stays what it is.

**An unloaded image was judged by its own box.** Until a picture arrives it has
no size of its own, so a page reserving a small box for it reports one: a
`min-width`/`min-height` placeholder, an unfired advert slot and an avatar are
the same measurement, and read one at a time a column of unloaded panels is a
column of icons. `LazyImageRuns` judges by **arrangement** instead — reading
content arrives as a stacked run of same-width boxes each starting where the
one above it ended, while an advert rail is slots thousands of pixels apart and
a related-items grid is several boxes sharing one vertical position. Geometry
only: no hostname, no selector, no class name.

**What was deliberately left alone.** The rule is **traversal's, never
selection's** — it answers "keep waiting here", not "save this". An image still
a placeholder when the page settles has nothing to store and
`selectImageCandidates` rejects it exactly as before, so the traversal set
stays a superset of the selected one, and `test/image_candidates_test.dart`
asserts that superset property directly. `AssetFetcher` still does not import
`capture_policy.dart`, and the image-only MIME allow-list is untouched.

**Tests:** `test/asset_refusal_test.dart`, `test/image_candidates_test.dart`,
`integration_test/capture_integrity_test.dart`.

### 17.7 The manifest records a rendering, and URLs gained an origin — 2026-09-03

**Files:** `lib/storage/manifest.dart`, `lib/core/url_utils.dart`.

Both changes serve the rendered fallback (V2-D74) and both are additive.

**`manifest.renderedFromPage`** is a durable optional flag saying the package
holds a rendering of the page rather than the site's own files. It is *not* an
`ArtifactFormat` and deliberately not read as one: the artifact still decides
how an entry is read, and a rendering of an image sequence is read as an image
sequence. The flag answers a different question — whether a re-save with real
files would be an improvement — and a manifest written without it reads exactly
as it did before, so §6's version rules are untouched.

**`url_utils.dart`** gained the origin (`scheme://host[:port]`) and
registrable-domain helpers `asset_origins` is keyed and scoped by. No existing
normalisation rule moved: `www.` is still not stripped, `http`/`https` are
still not unified, and `pageIdentityKey` is unchanged. The domain helper is
deliberately conservative — this app ships no public-suffix list, so it never
reaches past a page's own domain.

**Tests:** `test/rendered_capture_test.dart`, `test/asset_origin_learning_test.dart`,
`test/url_utils_test.dart`, `integration_test/live_rendered_fallback_test.dart`.

### 17.8 The bridge reads the page's prose, not its menus — 2026-09-03

**File:** `lib/browser/bridge_script.dart`.

Two measurement corrections, found on real readers, both inside §14's stated
device assumption that `innerText` is the browser's own layout-defined answer.

**`innerText` is trusted only where it is that answer.** The standard defines
`innerText` as the layout-aware reading *only for an element being rendered*;
for one that is not, it returns the welded `textContent`, raising nothing and
leaving no trace in the string. `probe` reads every `a[href]` on the page, not
only the ones on screen, and a listing routinely carries rows with no layout
box — a card hidden at this width, a panel behind an inactive tab. Those came
back glued: a label and a metadata span with no whitespace between them read as
one number, so entry 214 was discovered as entry 2146, and the digit it gained
was the row's age. `elementText` now asks whether the element has a layout box
(`getClientRects().length > 0`) before trusting `innerText`, and falls to
`joinTextNodes` otherwise — which over-separates rather than glues, which is
§14's stated safe direction. **The parser is deliberately untouched:** "Entry
2146 minutes" does not contain the information that the entry is 214, and
`test/entry_number_test.dart` pins the rule that a producer fault is never
papered over by guessing at digit runs.

**A page's own furniture is not its prose.** A page declaring no `<article>`,
no `<main>` and no dense paragraph container falls back to `document.body`, and
the region's reading dropped chrome *tags* only — so a site whose menus and
sign-in box are `<div>`s had all of it counted. `paragraphCount` was worse: its
selector ended in a bare `p`, so it counted every paragraph in the document.
One page measured 6,413 characters over 23 paragraphs of pure furniture,
classified as an article, and was handed to a text extraction whose own rules
then correctly found nothing — the two halves were answering different
questions. `proseSignals` is now the one answer both use: the visible,
non-hidden, non-furniture blocks `extractDocument` will keep. Second, a class
token saying a sidebar is *gone* was read as one being present —
`sidebar-hidden` on the readable column marked it furniture and emptied the
content-region image count with it. Chrome vocabulary is matched per class
token, a negation or absent-state qualifier beside the word denies it (judged
inside that token, so `visually-hidden` still matches), and the document root
is never furniture.

**What was deliberately left alone.** No hostname, selector or site list is
involved, and every threshold stays in Dart — the bridge measures, Dart
decides. The change *strengthens* filtering rather than loosening it: furniture
text stops passing as prose, so a text mode can no longer be offered for text
the extractor cannot read, and pages with real prose measure exactly as before.
Everything positional still derives from a single `metrics()` snapshot, so no
probe mixes two scrollers' coordinate systems; every call still ships its own
preamble and its own timeout. §14's listed gap stands — the measurement surface
is exercised mainly through device-bound suites, which is why both of these
surfaced on a device.

**Tests:** `test/bridge_element_text_test.dart`,
`test/bridge_prose_measurement_test.dart`, `test/content_detection_test.dart`,
`test/entry_number_test.dart`, `integration_test/text_capture_test.dart`.
