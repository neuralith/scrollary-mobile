...
| Field | Note |
|---|---|
| `host`, `path_key` | Source identity. `path_key` is today's `collectionFingerprint` output, and is `/` for a site given over to one work (V2-D72) |
...
| **I18** | `(host, path_key)` is unique within a **library**. One site section is one Source, under one Collection. `host` is folded to lower case; `path_key` is not (V2-D73) |
...
| Source identity | Source | `host + path_key`, library-wide (I18) | Client, deterministic |
...
A `path_key` of `/` — a site given over entirely to one work — is a Source
identity like any other, but it is the one whose key is a prefix of every
address on its host. What belongs to it is therefore decided by the same
derivation that produced the key (`addressKeysRoot`), never by a path prefix:
an address either keys the root or is not on that Source. Both the forward
walk and the listing reader ask that one question, and a bare host keys
nothing at all (V2-D72).
...
| `collections` | User | `folder_id NOT NULL`, `preferred_source_id`, `lifecycle`, ordering basis, and the two answers about the *work* that travel with it — `capture_mode`, `entry_sort` (V2-D73) |
| `sources` | Source | `UNIQUE(host, path_key)` — **library-wide, no Collection in it** (I18) — plus `CHECK (host = lower(host))`; language, lifecycle, `resolved_into_source_id` |
| `entries` | User | `collection_id` nullable, `folder_id` nullable, CHECK enforcing **I3**, `ordinal`, `placement` |
| `locations` | Source | `UNIQUE(url_key)`, `entry_id`, `source_id` nullable, source label and number, `published_at`, lifecycle |
...
| `collection_check_states` | **Device** | What this device's last update check of a Collection concluded — when, and whether it failed. Never the run in progress and never what it found (V2-D50 as V2-D73 amended it) |
| `asset_origins` | **Device** | What this device has watched an asset origin do, keyed by `scheme://host[:port]`: `suspected` after one refused reading, `refusing` after two on separate Locations, stale after a fortnight. Learned, never seeded (V2-D74) |
| `page_hints`, `saved_sites`, `favicons`, `settings` | **Device** | Carried over unchanged |

**Why `sources` is keyed without its Collection.** A site publishes one work,
and which work it is is the user's answer (V2-D45) — so `(host, path_key)`
appearing under two Collections is a contradiction rather than a row to keep.
The service enforces the same scope as `UNIQUE (library_id, host, path_key)`;
a device holds one library, so the local index needs no library column. Case is
asymmetric on both sides: the host is folded because DNS is case-insensitive,
the path key is not because RFC 3986 paths are (V2-D73, V2_SYNC.md §4.4a).
