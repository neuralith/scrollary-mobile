...
### 4.4a Source identity, and a site that moved

`(host, path_key)` is a Source's identity and it is unique **per library**
(I18) — not per Collection: a site publishes one work, and which work is the
user's answer (V2-D45). Both halves enforce the same scope, so neither can hold
a state the other cannot: a service that allowed two Collections to claim one
site section would feed a device a row its own index refuses, and stall the
pull on something nobody could fix.

**Case is asymmetric, deliberately.** The host is folded to lower case, because
DNS is case-insensitive and two spellings are one host. The path key is *not*,
because RFC 3986 paths are case-sensitive and `/Part` and `/part` may be two
places. `normalizeUrl` has always done exactly this; the schema now says so in
a CHECK, and the service folds every host it stores.

**A site that moved points forward.** `lifecycle: resolvedInto` with
`resolved_into_source_id` is the relocation (V2-D14): the old row stays and
names its successor. Both resolvers follow that chain to the Source being read
now — `CollectionRepository.terminalSourceOf` on the device and the arbitrator
on the service — with the same refusal to walk a loop. A `resolvedInto` row with
no destination is its own terminal: the lifecycle alone says only "not read here
any more", and the row is still the best answer there is.
...
### 4.7a A publication date is a Location's, and it crosses

`locations.published_at` is what **one Source** printed, kept beside
`source_label` and `source_number` because it is the same kind of thing:
evidence, never identity. Two Sources of one Collection may print different days
for the same work, so there is no single Entry-level answer for the model to
store — an Entry-level column would have to pick one and call it the truth.

It synchronises, and it has to: `ordering_basis: publicationDate` was already
library state that crossed, so a Collection could be ordered by publication date
on one device and by nothing at all on another. **The Entry-level reduction is
the reader's**, and it is ordered by `(discovered_at, id)` — both synchronised
columns — so two devices holding the same Locations reduce them to the same
date. A tie broken by whatever the database happened to return would be the one
remaining way this list could disagree with itself across devices.

### 4.8 What a Collection carries about itself

Two answers the user gave about the *work* travel with it, as opaque tokens the
service stores and never interprets (V2-D73):

| Answer | Column | Why it travels |
|---|---|---|
| What it is normally saved as (V2-D53, V2-D61) | `collections.capture_mode` | Someone who said "always images" said it about the Collection, not about the phone they said it on |
| What order its Entries are drawn in | `collections.entry_sort` | A list arranged by hand going back to its default on another device is the arrangement being lost |

`""` is unset, which is a question nobody has answered rather than a default. A
client that does not recognise a stored value **treats it as unset** — never as
some other value — which is what lets an older build hold a newer one's answer
without destroying it, and what makes a new sort field a client release instead
of a backend one.

**What happens to a finished Entry's files does NOT travel** (V2-D59). It
decides what happens to bytes on one device, offline copies never sync (I14),
and a phone deciding to remove would be deciding for a tablet with room to keep.
It stays in `local_settings`.

### 4.9 What never crosses the network
...
check timestamps and what a check found (`collection_check_states`; it persists
locally so a device that checked five minutes ago does not say *Not checked
yet*, and it is still a fact about **this** device) · the finished-copy rule
(§4.8) · derived Collection pointers · favicons · settings.

Two more that arrived with the rendered fallback (V2-D74), both device-local
for the same reason and neither expressible as an intent:

- **What this device has watched an asset origin do** (`asset_origins`). A
  verdict is an observation one device made over one device's network, at one
  moment; it goes stale after a fortnight and the only thing it ever changes is
  how many times the app asks again before believing an answer it already has.
  A phone behind a network that is challenged has learned nothing about a
  tablet on another.
- **Whether the user consented to a rendered copy from a Source.** It is an
  answer about what may be kept *on this device*, beside the finished-copy rule
  and for the same reason (§4.8), and it lives in `local_settings`.
...
### 8.1a Push is strict, pull is tolerant — and what follows from it

The two halves of sync answer an unrecognised field differently, on purpose,
and the asymmetry is the whole of this design's version story.

| Direction | An unknown field | Why |
|---|---|---|
| **Push** (`POST /mutations`) | **Rejected**, `validation_failed` | Silently dropping a field is how two clients drift apart with nobody noticing. The merge is an allowlist (`sync.MutableFields`) |
| **Pull** (`GET /changes`) | **Ignored** | A service that learned a field must not stall every client that has not; an unknown *entity kind* is skipped for the same reason |

Two consequences the codebase depends on:

- **A newer service does not break an older client.** It may add fields and
  even entity kinds; older devices read past them.
- **A newer client against an older service loses work.** The push is rejected,
  and a rejection is deterministic — so `OutboxRepository.markRejected` parks
  the row rather than retrying it forever. That intent is a user's change, and
  it is gone.

**The rollout rule, therefore: the service ships first.** Deploy a contract
change to the service, confirm it, and only then release the client that sends
the new field. There is deliberately **no capability negotiation** — a
single-service deployment does not need one, and a handshake nobody can test
against a second version is machinery pretending to be safety. If clients ever
face more than one service version at a time, `GET /version` is where that
negotiation goes, and this paragraph is the reason it would be added.

**What keeps the two halves honest in the meantime** is a pair of tests rather
than a promise. `internal/sync/vocabulary_test.go` holds the service's allowlist
against `contracts/openapi.yaml`; `test/sync/support/contract_vocabulary.dart`
reads the same file and the app's fake service applies it, so every push test is
a parity test. A contract change that updates one half fails a test rather than
someone's outbox.
