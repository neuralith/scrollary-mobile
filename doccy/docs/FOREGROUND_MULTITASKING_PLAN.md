...
**Later still, at `faf1379`, during a documentation audit** — again not a
re-validation of the feature, and no integration suite was run:

| Command | Result |
|---|---|
| `flutter test` (deterministic suite) | **2325 passed, 8 skipped, 2 failed.** Both failures are `repository_cleanliness_test.dart`, and both are environmental: the test walks the filesystem, and a **live, locked git worktree** at `.claude/worktrees/harness-cleanup` puts a second copy of the repository's own test files under a path the allowlist keys (`test/capture_policy_test.dart`, `test/collection_identity_test.dart`, …) cannot match. Every flagged path is inside that worktree; no file in the working tree is implicated |
| `flutter analyze` | **PASSED** — no issues |
| `dart format --output=none --set-exit-if-changed lib test integration_test tool` | **FAILED** — 25 of 418 files would change. Pre-existing and untouched by the audit, which edited no Dart file |
| `go build ./... && go vet ./...` (backend) | **PASSED** |
| `go test -count=1 ./...` (backend, Postgres via testcontainers) | **PASSED** — every package, including `internal/sync`'s contract-vocabulary test |
| `flutter test test/sync/contract_parity_test.dart` | **PASSED** — 3 cases; the app's half of the same contract allowlist |
