...
| [V2_CAPABILITY_PARITY.md](./V2_CAPABILITY_PARITY.md) | **Every capability that must stay reachable from app launch**, and the rule protecting it: an implementation may go only when a durable decision retires its capability, or an equivalent surface exists, is reachable, and its parity test passes |
| [V2_PORT_CHECKLIST.md](./V2_PORT_CHECKLIST.md) | The ported-verbatim components, their device assumptions and their test coverage. **§17 is the register of internal changes to them** — the rule is that changing a ported component's internals is a task with its own review, never a side effect of a call-site edit |
| [V2_UX_RECOVERY_TODO.md](./V2_UX_RECOVERY_TODO.md) | **Historical — all ten items are done.** The reading and capture regressions the V2 rewrite introduced, each with its root cause. Kept for the root causes, which are why several standing rules exist; not a plan |
...
1. The repository instructions in
   [scrollary-mobile/CLAUDE.md](../scrollary-mobile/CLAUDE.md) — the standing
   product and engineering rules — and the workspace's own cross-cutting rules
